...
| The editor stores no chosen kind as no field, one as a word, and several as a list. | `tests/src/features/edit-record/editDraft.test.ts` "drops the field when no kind is chosen" | listed cases |

...
| `npm test` | Vitest suites for the loader, the door, exports, and identity/palette seeds, in a shuffled order under a seed the run prints |
...
- **Read [docs/INVARIANTS.md](docs/INVARIANTS.md) before changing what the panel
...
- **The layer rule is absolute.** Imports point downward through
  ...
  dependencies. See
  [decision 0004, Build the panel as one-way sliced layers](docs/decisions/0004-build-the-panel-as-one-way-sliced-layers.md).
- **Content is checked at the door.** Everything entering the record passes through
  ...
  panel writes what it reads, so an unchecked value becomes a committed file. A saved copy
  the door refuses, or one that outlived a changed seed, is named in a notice on the panel.
  See [decision 0006, Guard the record with hand-written validators](docs/decisions/0006-guard-the-record-with-hand-written-validators.md)
  and [decision 0022, Name a refused or stale saved copy on the panel](docs/decisions/0022-name-a-refused-or-stale-saved-copy-on-the-panel.md).
...
- **Nothing prints from product code**, and the linter gates the rule with the scripts
  ...
  alert, or the notice that names a saved copy, never the console.
- **Suites mirror the source tree and substitute only at a seam.** One suite per unit under
  ...
  [decision 0010, Adopt the style's test contract](docs/decisions/0010-adopt-the-styles-test-contract.md).
...
| [docs/INVARIANTS.md](docs/INVARIANTS.md) | What must stay true about the panel, one row per claim with the holder that refuses a violation and the rung that says what its green is worth. Read before changing what the panel promises; a change to a claim changes its row. |
